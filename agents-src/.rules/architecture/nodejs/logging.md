...
description: pino setup, mandatory first-line debug log, outcome logs, and sensitive data rules.
---
...
The first line of every function must be a `log.debug` call with the function name as the second argument. No exceptions.
...
Add a `log.debug` when a function produces a meaningful result: a record persisted, a value computed, a decision made, an external call that returned a result.
